...
exports.getHomeWork = async (req, res) => {
    ...
    let pageNum = parseInt(req.query.pageNum) - 1;
    let pageSize = parseInt(req.query.pageSize);
    const sql = 'select t1.work_id,t1.work_name,t1.work_deadline,t1.work_time,t1.end_time,t1.work_details,t2.teacher_name,t3.course_name from homework t1 left join teacher t2 on t1.teacher_id = t2.teacher_id join course t3 on t2.teacher_id = t3.teacher_id join score t4 on t3.course_id = t4.course_id  where t4.student_id = ? and t1.work_status = 1 order by t1.work_time desc limit ?,?';
    const sql1 = 'select count(*) as total from homework t1 left join teacher t2 on t1.teacher_id = t2.teacher_id join course t3 on t2.teacher_id = t3.teacher_id join score t4 on t3.course_id = t4.course_id left join workinfo t5 on t1.work_id = t5.work_id where t4.student_id = ? and t1.work_status = 1';
    const sql2 = 'select * from workinfo where student_id = ? limit ?,?';
    db.query(sql, [student_id, pageNum * pageSize, pageSize], (err, results) => {
        if (err) {
            ...
            db.query(sql1, student_id, (err1, total) => {
                if (err1) {
                    return res.cc(err1.message);
                } else {
                    db.query(sql2, [student_id, pageNum * pageSize, pageSize], (err2, result2) => {
                        if (err2) {
                            return res.cc(err2.message);
                        } else {
                            let newResult = [];
                            if (result2.length > 0) {
                                newResult = results.map(item => {
                                    const obj = { ...item };
                                    const finishArr = result2.filter(i=>i.work_id === item.work_id);
                                    obj.finishInfo = finishArr;
                                    return obj
                                })
                                newResult.map(item=>{
                                    if(!item.finishInfo[0]){
                                        item.finishInfo.push({finishStatus:0});
                                    }
                                })
                            } else {
                                newResult = results;
...
exports.getDetailsHW = async (req, res) => {
    ...
    const sql = 'select work_id,work_name,work_details,end_time from homework  where work_id = ?';
